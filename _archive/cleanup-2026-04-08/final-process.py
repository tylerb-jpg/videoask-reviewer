...
def main():
    ...
    # Get contacts
    data = api_get(f'https://api.videoask.com/forms/{FORM_ID}/contacts?limit=20')
    if not data:
    ...
    for c in new_contacts:
        ...
        phone = c.get('phone_number', c.get('contact_phone_number', ''))
        
        # Get transcripts
        transcripts = {}
        for qname, qid in QUESTION_IDS.items():
            data = api_get(f'https://api.videoask.com/questions/{qid}/answers?limit=50')
            if data:
                ...
                if matching:
                    ...
                    # Update contact info from first answer
                    if not name:
        ...
        if phone:
            ...
            if len(digits) >= 3:
                area = digits[:3]
                if area in ['801','385','435']: market = 'utah'
                elif area in ['404','470','678','770','762','706','912','229','478']: market = 'georgia'
                elif area in ['480','602','623','520','928']: market = 'arizona'
