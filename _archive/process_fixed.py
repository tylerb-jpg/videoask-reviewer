...
for i, candidate in enumerate(candidates):
    ...
    # Build denied note for column N
    ...
    row_values = [
        ...
        approved_note,  # M - plain text (formula would be ="...")
        denied_note,  # N
        candidate['exp_str'],  # O
        ...
        candidate['transcripts'].get('q3', ''),  # T
        candidate['transcripts'].get('q4', ''),  # U
        candidate['transcripts'].get('q5', ''),  # V
        candidate['transcripts'].get('q6', ''),  # W
        candidate['transcripts'].get('q7', ''),  # X
    ]
    
    print(f"Row values length: {len(row_values)}")
    
    # Append row
    # Convert row values to CSV string
    csv_values = ','.join([str(v) if v is not None else '' for v in row_values])
    cmd = ['gws', 'sheets', '+append', '--spreadsheet', '1ySI_0RG9HIZKIdNCtfqW_gZ_U0VGIxAGkog9Nhnt2Y0', '--values', csv_values]
    ...
    cmd = ['gws', 'sheets', 'spreadsheets', 'batchUpdate', '--params', '{"spreadsheetId": "1ySI_0RG9HIZKIdNCtfqW_gZ_U0VGIxAGkog9Nhnt2Y0"}', '--json', json.dumps(batch_update)]
